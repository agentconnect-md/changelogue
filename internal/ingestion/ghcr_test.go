...
func TestGHCRSourceName(t *testing.T) {
	src := NewGHCRSource(http.DefaultClient, "agentconnect-md/changelogue", "")
...
func TestGHCRFetchNewReleases(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v2/agentconnect-md/changelogue/tags/list":
			if got := r.URL.Query().Get("n"); got != "100" {
				t.Fatalf("n = %q, want 100", got)
			}
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"name":"agentconnect-md/changelogue","tags":["v1.0.0","latest"]}`))
		case "/v2/agentconnect-md/changelogue/manifests/v1.0.0":
			w.Header().Set("Content-Type", "application/vnd.oci.image.manifest.v1+json")
			_, _ = w.Write([]byte(`{"annotations":{"org.opencontainers.image.created":"2026-01-02T03:04:05Z"}}`))
		case "/v2/agentconnect-md/changelogue/manifests/latest":
			w.Header().Set("Content-Type", "application/vnd.oci.image.manifest.v1+json")
			_, _ = w.Write([]byte(`{"config":{"digest":"sha256:latest-config"}}`))
		case "/v2/agentconnect-md/changelogue/blobs/sha256:latest-config":
			w.Header().Set("Content-Type", "application/json")
	...
	src := NewGHCRSource(srv.Client(), "agentconnect-md/changelogue", "src-123")
	src.baseURL = srv.URL
	...
	if results[0].Repository != "agentconnect-md/changelogue" {
		t.Errorf("results[0].Repository = %q, want agentconnect-md/changelogue", results[0].Repository)
	}
	if got := results[0].Metadata["release_url"]; got != "https://ghcr.io/agentconnect-md/changelogue:v1.0.0" {
...
func TestGHCRFetchWithBearerChallenge(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v2/agentconnect-md/private/tags/list":
			if r.Header.Get("Authorization") == "" {
				w.Header().Set("WWW-Authenticate", `Bearer realm="`+tokenURL+`",service="ghcr.io",scope="repository:agentconnect-md/private:pull"`)
			...
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"name":"agentconnect-md/private","tags":["v2.0.0"]}`))
		case "/v2/agentconnect-md/private/manifests/v2.0.0":
			if r.Header.Get("Authorization") == "" {
				w.Header().Set("WWW-Authenticate", `Bearer realm="`+tokenURL+`",service="ghcr.io",scope="repository:agentconnect-md/private:pull"`)
				w.WriteHeader(http.StatusUnauthorized)
				return
		...
		case "/token":
			...
			if got := r.URL.Query().Get("scope"); got != "repository:agentconnect-md/private:pull" {
	...
	src := NewGHCRSource(srv.Client(), "ghcr.io/agentconnect-md/private", "")
	src.baseURL = srv.URL
	...
	if results[0].Repository != "agentconnect-md/private" {
...
func TestGHCRFetchTimestampFromManifestListAnnotations(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/v2/agentconnect-md/changelogue/tags/list":
			_, _ = w.Write([]byte(`{"name":"agentconnect-md/changelogue","tags":["multi"]}`))
		case "/v2/agentconnect-md/changelogue/manifests/multi":
			_, _ = w.Write([]byte(`{
				"manifests": [
	...
	src := NewGHCRSource(srv.Client(), "agentconnect-md/changelogue", "")
	src.baseURL = srv.URL
...
func TestGHCRFetchTimestampUnavailableDoesNotUseFetchTime(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/v2/agentconnect-md/changelogue/tags/list":
			_, _ = w.Write([]byte(`{"name":"agentconnect-md/changelogue","tags":["unknown"]}`))
		case "/v2/agentconnect-md/changelogue/manifests/unknown":
...
func TestGHCRPagination(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v2/agentconnect-md/changelogue/tags/list":
			tagsCallCount++
			if tagsCallCount == 1 {
				w.Header().Set("Link", `</v2/agentconnect-md/changelogue/tags/list?n=100&last=v1.0.0>; rel="next"`)
				_, _ = w.Write([]byte(`{"name":"agentconnect-md/changelogue","tags":["v1.0.0"]}`))
				return
			}
			if got := r.URL.Query().Get("last"); got != "v1.0.0" {
				t.Fatalf("last = %q, want v1.0.0", got)
			}
			_, _ = w.Write([]byte(`{"name":"agentconnect-md/changelogue","tags":["v2.0.0"]}`))
		case "/v2/agentconnect-md/changelogue/manifests/v1.0.0":
			_, _ = w.Write([]byte(`{"annotations":{"org.opencontainers.image.created":"2026-01-01T00:00:00Z"}}`))
		case "/v2/agentconnect-md/changelogue/manifests/v2.0.0":
	...
	src := NewGHCRSource(srv.Client(), "agentconnect-md/changelog", "")
	src.repository = "agentconnect-md/changelogue"
	src.baseURL = srv.URL
